...
class PlayState extends MusicBeatState
{
	...
	public static var snickerNoMiss:Bool = false;
	public static var butterNoMiss:Bool = false;
	public static var toothNoMiss:Bool = false;
	public static var meltNoMiss:Bool = false;
	public static var snicker90Acc:Bool = false;
	public static var butter90Acc:Bool = false;
	public static var tooth90Acc:Bool = false;
	public static var melt90Acc:Bool = false;
	
	var turnedOff:Bool = false;
	
	var closedDial:Bool = false;
	
	var showRes:Bool = false;
	...
	public static var songScore:Int = 0; //for discord client in results
	...
	override public function create()
	{
		...
		if (isStoryMode)
		{
			detailsText = "Story Mode: Cavity Crush";
		}
		else
		{
		...
		switch (songLowercase)
		{
			...
			case 'snicker':
				dialogue = CoolUtil.coolTextFile(Paths.txt('dialogues/snickerDialogue'));
			case 'butterfingers':
				dialogue = CoolUtil.coolTextFile(Paths.txt('dialogues/butterDialogue'));
			case 'toothache':
				dialogue = CoolUtil.coolTextFile(Paths.txt('dialogues/toothDialogue'));
			case 'melt':
				dialogue = CoolUtil.coolTextFile(Paths.txt('dialogues/meltDialogue'));
		}
		...
		if (SONG.stage == null)
		{
			switch (storyWeek)
			{
				...
				case 7:
					if (songLowercase == 'melt')
					{
						stageCheck = 'dessertCityRuins';
					}
					else
					{
						stageCheck = 'dessertCity';
					}
		...
		switch (Stage.curStage)
		{
			...
			case 'dessertCityRuins':
				//i wanted to add the evil trails seen in week 6 (literally above the dessert city code) but that made the idle animations go wayyy too fast
				boyfriend.x += 200;
				dad.x -= 100;
		}
		...
		//gonna do the same lmao	
		var orangeShit:FlxSprite = new FlxSprite(0, 0).makeGraphic(FlxG.width * 6, FlxG.height * 6, FlxColor.ORANGE); 
		orangeShit.alpha = 0.25;
		orangeShit.screenCenter();
		if (Stage.curStage == 'dessertCityRuins')
			add(orangeShit);
		...
		else
		{
			...
			{
				default:
					DialogueBox.dialogueClosed = true;
					startCountdown();
			}
		}
	...
	{
		var black:FlxSprite = new FlxSprite( -100, -100).makeGraphic(FlxG.width * 16, FlxG.height * 16, FlxColor.BLACK);
		black.screenCenter();
		...
		if (songLowercase == 'roses' || songLowercase == 'thorns' || songLowercase == 'butterfingers' || songLowercase == 'toothache' || songLowercase == 'melt')
		{
			remove(black);
	...
	override public function update(elapsed:Float)
	{
		...
		if (SONG.song.toLowerCase() == 'melt')
		{
			if (FlxG.save.data.distractions)
			{
				healthBar.y += Math.cos(0.6 + floatshit) / 6;
				healthBarBG.y += Math.cos(0.6 + floatshit) / 6;
				iconP1.y += Math.cos(0.6 + floatshit) / 6;
				iconP2.y += Math.cos(0.6 + floatshit) / 6;
				botPlayState.y += Math.cos(1.2 + floatshit) / 6;
				scoreTxt.y += Math.cos(1.2 + floatshit) / 6;
			}
		}
		
		if (SONG.song.toLowerCase() == 'melt' && FlxG.keys.justPressed.L && !paused && DialogueBox.dialogueClosed && !SONG.notes[Math.floor(curStep / 16)].mustHitSection)
		{
			lCount++;
			trace("L #" + lCount);
			if (lCount == 100 && !FlxG.save.data.fall)
			{
				FlxG.save.data.fall = true;
				FlxG.save.flush();
				trace("big bruh moment");
				unlock.play();
			}
		}
		...
		{
			trace("Code Active: " + TitleState.cheatActive);
			unlock.play();
		}	
		...
		else
		{
			...
			else
			{
				if (isStoryMode)
				{
					...
					if (SONG.song == 'Snicker' && misses == 0 && !FlxG.save.data.botplay)
					{
						snickerNoMiss = true;
						trace("da Snicker wit no misses.");
					}
					
					if (SONG.song == 'Butterfingers' && misses == 0 && !FlxG.save.data.botplay)
					{
						butterNoMiss = true;
						trace("da Butterfingers wit no misses.");
					}
					
					if (SONG.song == 'Toothache' && misses == 0 && !FlxG.save.data.botplay)
					{
						toothNoMiss = true;
						trace("da Toothache wit no misses.");
					}
					
					if (SONG.song == 'Melt' && misses == 0 && !FlxG.save.data.botplay)
					{
						meltNoMiss = true;
						trace("da Melt wit no misses.");
						if (toothNoMiss && snickerNoMiss && butterNoMiss)
						{
				...
				else
				{
					...
					else
					{
						...
						{
							if (accuracy >= 90 && storyDifficulty == 2)
							{
	...
	*/
	...
	{
		if (curSong.toLowerCase() == 'butterfingers')
		{
			if (curBeat % gfSpeed == 0) //why make new variable when gf has one
				danced = !danced;
		}
		else
		{
		...
		//more beat zooms for 
		if (curSong.toLowerCase() == 'melt' && camZooming && FlxG.camera.zoom < 1.35 && curBeat % 2 == 0)
		{
			FlxG.camera.zoom += 0.015;
			if (curBeat % 1 == 0)
				camHUD.zoom += 0.03;
		...
		switch (Stage.curStage)
		{
			...
			case "dessertCity":
				if (!showRes)
				{
					if (curStep % bopSpeed == 0) //curbeat was too slow so curstep it is
					{
						if (FlxG.save.data.distractions)
						{
							for (bg in Stage.animatedBacks)
			...
			case "dessertCityRuins":
				if (curStep % bopSpeed == 0) //doesnt need a showRes since the screens already black
				{
					if (FlxG.save.data.distractions)
					{
						for (bg in Stage.animatedBacks)
