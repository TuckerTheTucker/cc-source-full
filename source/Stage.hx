            ...
            case 'halloween':
				...
				case 'dessertCity':
						...
						*/
						...
						var tree = new FlxSprite( -250, 250);
						tree.frames = Paths.getSparrowAtlas('dstage/pinetree');
						tree.animation.addByPrefix('idle', 'treebop', 24, false);
						tree.setGraphicSize(Std.int(tree.width * 1.2));
						...
						var tawog = new FlxSprite(25, 350);
						tawog.frames = Paths.getSparrowAtlas('dstage/gumballmachine');
						tawog.animation.addByPrefix('idle', 'ballbop', 24, false);
						tawog.setGraphicSize(Std.int(tawog.width * 1.1));
						tawog.scrollFactor.set(.95, 1);
						if (FlxG.save.data.distractions)
						{
						    swagBacks['tawog'] = tawog;
                            toAdd.push(tawog);
                            animatedBacks.push(tawog);
						}
						
						var deskman = new FlxSprite(1350, 250);
						deskman.frames = Paths.getSparrowAtlas('dstage/manbop');
						deskman.animation.addByPrefix('idle', 'table', 24, false);
						deskman.setGraphicSize(Std.int(deskman.width * 1.2));
				...
				case 'dessertCityRuins':
					...
					/*
                        ...
                        toAdd.push(tempbg);
					*/
						var outside:FlxSprite = new FlxSprite(10, -150).loadGraphic(Paths.image('dstageruins/outside'));
						outside.scrollFactor.set(.8, .8);
						outside.setGraphicSize(Std.int(outside.width * 1.05));
						outside.screenCenter(X);
						outside.active = false;
						swagBacks['outside'] = outside;
                        toAdd.push(outside);
						
						var bg:FlxSprite = new FlxSprite(-1400, -600).loadGraphic(Paths.image('dstageruins/wallz'));
						bg.scrollFactor.set(1, 1);
						bg.active = false;
						swagBacks['bg'] = bg;
                        toAdd.push(bg);
						
						var tree = new FlxSprite( -250, 350);
						tree.frames = Paths.getSparrowAtlas('dstageruins/pinetree');
						tree.animation.addByPrefix('idle', 'treebop', 24, false);
						tree.setGraphicSize(Std.int(tree.width * 1.2));
						tree.scrollFactor.set(.95, 1);
						if (FlxG.save.data.distractions)
						...
						var tawog = new FlxSprite(25, 350);
						tawog.frames = Paths.getSparrowAtlas('dstageruins/gumballmachine');
						tawog.animation.addByPrefix('idle', 'ballbop', 24, false);
						tawog.setGraphicSize(Std.int(tawog.width * 1.1));
						tawog.scrollFactor.set(.95, 1);
						if (FlxG.save.data.distractions)
						...
						var deskman = new FlxSprite(1350, 350);
						deskman.frames = Paths.getSparrowAtlas('dstageruins/manbop');
						deskman.animation.addByPrefix('idle', 'manbop', 24, false);
						deskman.setGraphicSize(Std.int(deskman.width * 1.2));
						deskman.scrollFactor.set(.95, 1);
						if (FlxG.save.data.distractions)
						{
